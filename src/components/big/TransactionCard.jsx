...
const TransactionCard = ({ navigation }) => {
    ...
    const getdata = async () => {
        ...
        if (token) {
            const response = await axios.post('https://stayholic.com/api/v1/customer/statement ', {
                token, token
            },
                {
                    headers: {
            ...
            // Handle the response
            console.log(response.data[0].wallet_transactions);
            setWalletData(response.data[0].wallet_transactions)
